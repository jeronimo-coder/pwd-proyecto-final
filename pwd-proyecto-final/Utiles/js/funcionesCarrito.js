...
function armarTablaCarrito(arreglo) {
    ...
    if ((arreglo.length) > 0) {
        ...
        $('#tablaCarrito').append('<thead class="table-dark"><tr><th hidden>' + idcompra + '</th><th class="fw-bold">Producto</th><th class="fw-bold">Precio</th><th class="fw-bold">Cantidad</th><th class="fw-bold">Subtotal</th><th width="150"><a href="#" class="vaciarCarrito"><button class="btn btn-danger">Vaciar</button></a></th></tr></thead><tbody class="table-striped"></tbody>');


        var total = 0;
        $.each(arreglo, function(index, compraItem) {
            total += compraItem.subtotal;
        
            let newRow = `<tr>
                <td hidden>${compraItem.idcompraitem}</td>
                <td hidden>${compraItem.idproducto}</td>
                <td hidden>${compraItem.idcompra}</td>
                <td hidden>${compraItem.procantstock}</td>
                <td>
                    <img src="./img/productos/${compraItem.imagen}" class="rounded float-start" width="150" height="150">
                    <p>${compraItem.pronombre}</p>
                    <p>${compraItem.detalle}</p>
                </td>
                <td>
                    <p><strong>$ ${compraItem.precio} ARS</strong></p>
                </td>
                <td>
                    <input min="1" max="${compraItem.procantstock}" type="number" class="form-control procantstock" name="procantstock" autocomplete="off" value="${compraItem.cicantidad}">
                </td>
                <td>
                    <p class="h3 text-info"><strong>$ ${compraItem.subtotal} ARS</strong></p>
                </td>
                <td>
                ...
                <td hidden>${compraItem.cicantidad}</td>
            </tr>`;
        
            $('#tablaCarrito > tbody:last-child').append(newRow);
        });

        $('#totalPagar').append('<div class="card text-center h2 bg-dark text-white" style="width: 18rem;"><div class="card-body"><h5 class="card-title">Total a pagar:</h5><hr><p class="card-text"> $ ' + total + ' ARS</p><a href="#" class="pagar btn btn-success">Pagar</a><br><br></div></div>')

    } else {

        $('#estructuraCarrito').append('<div class="card mx-auto" style="max-width: 540px;"><div class="row g-0"><div class="col-md-4"><img src="./img/logoGomeria.png" width="200" height="200" alt="..."></div><div class="col-md-8"><div class="card-body"><h4 class="card-title">Carrito de compras vac&iacuteo</h4><p class="card-text">A&uacuten no has agregado productos al carrito. Ingresa al cat&aacutelogo de productos para seleccionar los que mas te gusten.</p><p class="card-text"><a href="./productos.php"><button class="btn btn-outline-success col-11">Cat&aacutelogo de productos</button></a></p></div></div></div></div>');
    }
