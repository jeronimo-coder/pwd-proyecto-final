...
function armarCambioRol(roles) {
    ...
    if (Object.keys(roles).length > 1) { //Si tiene más de un rol
        cambioRolCuerpo += "<!-- INICIO CAMBIOS ROL --><li class='nav-item dropdown'><a class='nav-link dropdown-toggle' href='#' data-bs-toggle='dropdown'>Roles</a><ul class='dropdown-menu dropdown-menu-end'>";
...
function armarDatosUsuario(usuario) {
    let usuarioCuerpo = `
        ...
        <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle" href="#" data-bs-toggle="dropdown">${usuario.nombre}</a>
            <ul class="dropdown-menu dropdown-menu-end">
                <li>
                    <button class="dropdown-item h4">
                        ${usuario.rol !== null ? `Rol Actual: ${usuario.rol}` : "No tienes roles"}
                    </button>
                </li>
                <li>
                    <a class="dropdown-item h5" href="../Acciones/login/cerrarSesion.php">
